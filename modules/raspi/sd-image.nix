# Modified from https://github.com/NixOS/nixpkgs/blob/663f1d8ba2753ce68fcf7f9ef8198b4c35d13558/nixos/modules/installer/sd-card/sd-image.nix
...
in
{
  imports = [ ./file-options.nix ];

  options.sdImage = {
    storePaths = mkOption {
      type = with types; listOf package;
      example = literalExpression "[ pk.gs.stdenv ]";
      description = ''
  ...
  config = {
    fileSystems."/boot/firmware".device = "/dev/disk/by-label/${config.sdImage.firmwarePartitionName}";
    fileSystems."/boot/firmware".fsType = "vfat";
    fileSystems."/boot/firmware".options = [
      "nofail"
      "noauto"
    ];
    fileSystems."/".device = "/dev/disk/by-label/${config.sdImage.rootVolumeLabel}";
