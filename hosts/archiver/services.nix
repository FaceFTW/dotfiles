{ pkgs, ... }:
{
  ...
  systemd.services.nix-serve = {
    ...
    script = ''
      ...
      exec ${pkgs.nix-serve}/bin/nix-serve --listen localhost:5000
    '';
    ...
    serviceConfig = {
      ...
      Group = "nix-serve";
      DynamicUser = true;
      LoadCredential = "NIX_SECRET_KEY_FILE:/run/secrets/nix_cache_priv_key";
      BindPath = "/mnt/motorway/nix/store:/nix/store";
    };
  };
  ...
  systemd.services.ugreen-led-mons = {
    ...
    script = ''
      ...
      MAPPING_METHOD=ata # ata, hctl, serial
      led_map=(disk1 disk2)
      ...
      function disk_enumerating_string() {
          if [[ $MAPPING_METHOD == ata ]]; then
              ls -ahl /sys/block | sed 's/\/$//' | ${pkgs.gawk}/bin/awk '{
                  if (match($0, /ata[0-9]+/)) {
